...
export function JobTrackerProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const loadPersistedJob = async () => {
            try {
                ...
                if (stored) {
                    const parsed = JSON.parse(stored) as { jobId: string; params?: JobParams };
                    if (parsed.jobId) {
    ...
    const startJob = useCallback(async (params: JobParams): Promise<string | null> => {
        try {
            ...
            if (!response.ok) {
                const error = await response.json().catch(() => ({})) as GenerationJobApiResponse;
                const reasonCode = typeof error.reason_code === 'string' ? error.reason_code : '';
                const message = typeof error.error === 'string' ? error.error : 'Failed to start job';
                throw new Error(reasonCode ? `${message} (${reasonCode})` : message);
            }
