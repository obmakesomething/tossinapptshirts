  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/v1/address/search?query=${encodeURIComponent(query)}`,
      );
      if (!response.ok) {
        const data = (await response
          .json()
          .catch(() => ({}) as AddressSearchResponse)) as AddressSearchResponse;
        throw new Error(
          data.error || '주소를 찾지 못했어요. 다시 검색해 주세요.',
        );
      }
  ...
  const mappedResults = useMemo(() => {
    return results.map((item) => {
      ...
      return {
        ...
        displaySub:
          jibunAddress && roadAddress && jibunAddress !== roadAddress
            ? jibunAddress
            : '',
        payload: {
          zonecode,
    ...
    >
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Button type="light" size="tiny" onPress={onClose}>
            닫을게요
          </Button>
        </View>
        ...
        {error ? (
          <View style={styles.errorBox}>
            <Txt typography="t7" color={theme.colors.error}>
              {error}
            </Txt>
          </View>
        ) : null}
          ...
          {mappedResults.map((item) => (
            <ListRow
              ...
              contents={
                <ListRow.Texts
                  type={
                    item.displaySub || item.payload.zonecode
                      ? '2RowTypeA'
                      : '1RowTypeA'
                  }
