...
import {
  type AddressData,
  DaumPostcodeModal,
} from '../components/DaumPostcodeModal';
import {
  Card,
  Chip,
  ColorSwatch,
...
function Page() {
  ...
  const blockingMessage = loginLoading
    ? '토스 로그인 중이에요...'
    : '결제를 준비하고 있어요...';
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = (await response
          .json()
          .catch(() => ({}) as PaymentErrorResponse)) as PaymentErrorResponse;
      ...
      console.log(
        '[Order] Login successful, userKey:',
        `${data.userKey?.substring(0, 10)}...`,
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (typeof payToken !== 'string' || !payToken) {
        throw new Error(
          '결제 토큰을 확인하지 못했어요. 잠시 후 다시 시도해 주세요.',
        );
      }
      ...
      const { success, reason } = await TossPay.checkoutPayment({
        params: { payToken },
      });
      ...
      const executeResponse = await fetch(
        `${API_BASE_URL}/v1/payment/execute`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  if (!userKey) {
    return (
      <>
        <Screen>
                ...
                <View style={styles.chipRow}>
                  {selectedProduct.sizes
                    .filter(
                      (size) =>
                        !orderLines.some(
                          (line) => line.sizeLabel === size.label,
                        ),
                    )
                    .map((size) => (
                      <Chip
          ...
          <Card style={styles.loginCard}>
            ...
            <Text style={styles.loginDesc}>
              안전한 결제를 위해 토스 계정으로 로그인해 주세요. 로그인하시면
              바로 주문을 진행할 수 있어요.
            </Text>
          </Card>
          ...
          <View style={styles.actionRow}>
            <PrimaryButton
              label={
                loginLoading ? '로그인 중...' : '🔐 토스 로그인하고 주문하기'
              }
        ...
        <FullScreenLoader visible={loginLoading} message={blockingMessage} />
      </>
    );
  }
    ...
    <>
      <Screen>
            ...
            <View style={styles.editSection}>
              ...
              <View style={styles.chipRow}>
                {selectedProduct.sizes
                  .filter(
                    (size) =>
                      !orderLines.some((line) => line.sizeLabel === size.label),
                  )
                  .map((size) => (
                    <Chip
                      key={size.label}
        ...
        <Text style={styles.noticeText}>
          출력 이미지는 주문자가 직접 확인해 주세요. 주문서 메일을 꼭 확인해
          주세요.
        </Text>
      ...
      <FullScreenLoader visible={submitting} message={blockingMessage} />
    </>
  );
}
