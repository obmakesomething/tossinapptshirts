...
배포: GCP Cloud Run `merchandisegpt-api-peaq3gmvyq-du.a.run.app`
...
6. 컨테이너 재시작/연결 끊김: `SIGTERM`, `connection closed unexpectedly` (배포/리스타트 영향).
...
## 체크리스트 (남은 확인 항목)
- GCP ENV 확인:
  - `GCS_UPLOAD_BUCKET` 설정 확인
  - `SIGNED_URL_TTL_SECONDS` 설정 확인
